...
| `first_project/`       | A Python directory that contains code for your Dagster repository                 |
| `first_project_tests/` | A Python directory that contains tests for `first_project`                      |
...
pyenv virtualenv $PYTHON_VERSION first_project
...
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name first_project python=PYTHON_VERSION
...
Tests can be found in `first_project_tests` and are run with the following command:

```bash
pytest first_project_tests
```

As you create Dagster ops and graphs, add tests in `first_project_tests/` to check that your
