...
import numpy as np
# https://realpython.com/python-dash/s
types = {
    'sbd': 'numeric',
    'tên': 'text',
    'dd': 'numeric',
...
df = pd.read_csv(r'D:\Python + SQL + R\Python_dunglai\clean_data.csv')
# df['id'] = df['Unnamed: 0']
...
app.layout = html.Div(
                children=[
                    html.Div(
                        children=[
                            html.Div(children="sbd", className="menu-title"),
                            dcc.Dropdown(
                                id="sbd-filter",
                                options=[
                                    {"label": sbd, "value": sbd}
                                    for sbd in np.sort(df.sbd.unique())
                                ],
                                value="Albany",
                                clearable=False,
                                className="dropdown",
