...
file_2  = open('cuong_test_all_data.txt',encoding='utf8',mode='w')
sbd = 2000000
for data in datas:
	sbd +=1
	...
	data = [sbd_str,name.title(), str(dd), str(mm), str(yy)] # chỉ lấy các thông tin liên quan
	...
	file_2  = open('cuong_test_all_data.txt',encoding='utf8',mode='a')
	for i in range(len(data)):
		file_2.write(data[i]+',')
